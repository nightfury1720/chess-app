...
export class Board {
    ...
    calculateAllMoves() {
        ...
        for (const piece of this.pieces) {
            if(this.currentTeam !== piece.team)continue;
    ...
    checkCurrentTeamMoves() {
        ...
        for (const piece of this.pieces.filter(p => p.team === this.currentTeam)) {
            ...
            //TODO: ADD logic to iterate ove all possible moves
            for (const move of piece.possibleMoves) {
                const simulatedBoard = this.clone();

                // Remove the piece at the destination position
                // simulatedBoard.pieces = simulatedBoard.pieces.filter(p => !p.samePosition(move));
                // TODO: ADD BitBoard logic to remove the killed piece availability from its bitboard
